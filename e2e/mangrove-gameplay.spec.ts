...
import { POISON_DURATION_SECONDS } from "../src/gameplay/model/poison";
...
test("Verband stoppt Blutungen und Heilkraut heilt Gift", async ({ page }) => {
  ...
  const result = await page.evaluate(() => {
    ...
    debug.use("bandage");
    const bandaged = debug.snapshot() as MangroveSnapshot;
    debug.poison();
    debug.use("healing_herb");
    const poisonCured = debug.snapshot() as MangroveSnapshot;
    return { bleeding, sick, cured, poisoned, bandaged, poisonCured };
  });

  expect(result.bleeding.conditions.isBleeding).toBe(true);
  expect(result.sick.survival.health).toBe(55);
  expect(result.bandaged.survival.health).toBe(75);
  expect(result.bandaged.conditions.poisonSecondsRemaining).toBe(0);
  ...
  expect(result.poisoned.conditions.poisonSecondsRemaining).toBe(POISON_DURATION_SECONDS);
  expect(result.poisonCured.conditions.poisonSecondsRemaining).toBe(0);
  expect(result.poisonCured.inventory.some(({ itemId }) => itemId === "simple_bandage" || itemId === "bandage" || itemId === "herbal_antidote")).toBe(false);
